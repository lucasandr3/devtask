<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <form action="{{ route('emails.toggle-star', $emailMessage) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="p-2 rounded-lg hover:bg-muted dark:hover:bg-gray-700 transition-colors ui-tooltip ui-tooltip-top" data-tooltip="{{ $emailMessage->is_starred ? 'Remover favorito' : 'Favoritar' }}" aria-label="{{ $emailMessage->is_starred ? 'Remover favorito' : 'Favoritar' }}">
                ...
                <form action="{{ route('emails.destroy', $emailMessage) }}" method="POST" class="inline" data-confirm="Tem certeza que deseja excluir este email?" data-confirm-title="Excluir email?">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-red-500 transition-colors ui-tooltip ui-tooltip-top" data-tooltip="Excluir email" aria-label="Excluir email">
    ...
    <div class="card">
        ...
        @if($emailMessage->attachments && count($emailMessage->attachments) > 0)
            <div class="p-6 border-t border-border border-border bg-muted/50 bg-card/50">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach($emailMessage->attachments as $attachment)
                        <div class="flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-700 rounded-lg border border-border dark:border-gray-600">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                            </svg>
                            <span class="text-sm text-foreground">{{ $attachment['name'] ?? 'Anexo' }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        {{-- Ações --}}
        <div class="p-6 border-t border-border border-border">
