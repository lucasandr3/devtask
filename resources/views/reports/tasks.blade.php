<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-gray-100 dark:bg-gray-700/30 rounded-xl shrink-0">
                        <svg class="w-6 h-6 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm text-gray-500 dark:text-gray-400">A Fazer</p>
                        <p class="text-2xl font-bold text-gray-600 dark:text-gray-400">{{ $todoTasks }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <p class="text-sm text-gray-500 dark:text-gray-400">Pull Requests</p>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Tarefas do Período</h3>
                        ...
                        @forelse($tasks as $task)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                    ...
                                    <span class="text-gray-500 dark:text-gray-400 font-normal">({{ $task->work_date->translatedFormat('D') }})</span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">
                                    <div class="font-medium">{{ $task->title }}</div>
                                    @if($task->description)
                                        <div class="text-gray-500 dark:text-gray-400 text-xs mt-1">{{ Str::limit($task->description, 100) }}</div>
                                    @endif
                                </td>
