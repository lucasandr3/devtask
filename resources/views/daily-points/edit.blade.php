<x-app-layout>
    ...
    <div>
        <div class="card p-6">
            <form method="POST" action="{{ route('horas.atualizar', $point) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input type="text" name="entry_time" id="entry_time" value="{{ old('entry_time', $point->entry_time ? $point->entry_time->format('H:i') : '') }}" class="mt-1" placeholder="08:00" maxlength="5" />
                        <x-input-error :messages="$errors->get('entry_time')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="lunch_out_time" value="Saída Almoço (HH:MM)" />
                        <x-text-input type="text" name="lunch_out_time" id="lunch_out_time" value="{{ old('lunch_out_time', $point->lunch_out_time ? $point->lunch_out_time->format('H:i') : '') }}" class="mt-1" placeholder="12:00" maxlength="5" />
                        <x-input-error :messages="$errors->get('lunch_out_time')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="lunch_return_time" value="Volta Almoço (HH:MM)" />
                        <x-text-input type="text" name="lunch_return_time" id="lunch_return_time" value="{{ old('lunch_return_time', $point->lunch_return_time ? $point->lunch_return_time->format('H:i') : '') }}" class="mt-1" placeholder="13:00" maxlength="5" />
                        <x-input-error :messages="$errors->get('lunch_return_time')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="exit_time" value="Saída (HH:MM)" />
                        <x-text-input type="text" name="exit_time" id="exit_time" value="{{ old('exit_time', $point->exit_time ? $point->exit_time->format('H:i') : '') }}" class="mt-1" placeholder="18:00" maxlength="5" />
                ...
                <div class="border-t border-border pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input type="text" name="extra_start_time" id="extra_start_time" value="{{ old('extra_start_time', $point->extra_start_time ? $point->extra_start_time->format('H:i') : '') }}" class="mt-1" placeholder="18:30" maxlength="5" />
                            <x-input-error :messages="$errors->get('extra_start_time')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="extra_end_time" value="Fim Hora Extra (HH:MM)" />
                            <x-text-input type="text" name="extra_end_time" id="extra_end_time" value="{{ old('extra_end_time', $point->extra_end_time ? $point->extra_end_time->format('H:i') : '') }}" class="mt-1" placeholder="20:00" maxlength="5" />
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3" class="input mt-1" placeholder="Observações sobre a hora...">{{ old('notes', $point->notes) }}</textarea>
